...
export default function ResultsCampaigns(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const config: ChartConfiguration = {
      ...
      options: {
        indexAxis: "y",
        maintainAspectRatio: false, // Use 'y' to make it a horizontal bar chart
      },
    };
  ...
  return (
    <div>
      <div className="flex flex-col md:flex md:flex-row justify-evenly items-center my-6">
        ...
        <div className="w-2/3 md:w-1/4 xl:w-1/3 h-[350px] m-2 xl:m-8 bg-white p-4 rounded-md border-2">
          <div className="flex justify-between font-medium">
            <p>Cost per app use</p>
            <p>Rs. 48</p>
          </div>
      ...
      <div className="flex flex-wrap gap-6 justify-evenly px-8 my-4">
        {[
          ["Clicks", 85],
          ["Impressions", 85],
          ["Average CPC", 85],
          ...
          <div className="flex flex-col w-1/3 sm:w-1/4 md:w-1/6 h-28 md:h-40 bg-white p-4 gap-4 justify-center rounded-md border-2">
            <p className="text-sm">{title}</p>
            <p className="text-4xl md:text-6xl font-medium text-center">{value}</p>
          </div>
        ))}
      </div>
    </div>
