...
export default function Delete({ deleteSender, onClose }: DeleteProps): JSX.Element {
  ...
  return (
    <div
      ...
      className={`fixed inset-0 flex justify-center items-center transition-colors
        ${deleteSender ? "visible bg-black/20" : "invisible"}`}
    >
      <div
        role="button"
        tabIndex={0}
        ...
        className={`bg-white rounded-xl shadow px-14 py-7 transition-all w-5/6  sm:w-2/3 lg:w-1/2
              ${deleteSender ? "scale-100 opacity-100" : "scale-125 opacity-0"}`}
      >
        <h1 className="text-center font-medium text-2xl mb-5">Delete a Sender</h1>

        <form onSubmit={handleSubmit}>
          <p>Active templates and scheduled campaigns should be updated with another sender.</p>
          <p>Are you sure you want to delete the sender ? </p>
